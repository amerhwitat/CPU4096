...
P=(-scheme "$SCHEME" -configuration Release)
if [[ "$PROJECT_OR_WORKSPACE" == *.xcworkspace ]]; then P=(-workspace "$PROJECT_OR_WORKSPACE" "${P[@]}"); else P=(-project "$PROJECT_OR_WORKSPACE" "${P[@]}"); fi
